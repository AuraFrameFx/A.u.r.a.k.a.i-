...
Characters can actually WALK around the screen!

```kotlin
// Aura casually strolls across from left to right
AnimatedCharacter(
    manifestation = auraManifestation,
    assetBitmap = auraAsset,
    behavior = MovementPresets.casualWalkAcross(fromLeft = true)
)

// Kai walks in from the left, pauses in center, walks out
AnimatedCharacter(
    manifestation = kaiManifestation,
    assetBitmap = kaiAsset,
    behavior = MovementPresets.walkInAndWave()
)

// Aura randomly wanders around the bottom area
AnimatedCharacter(
    manifestation = auraManifestation,
    assetBitmap = auraAsset,
    behavior = MovementPresets.randomWander(
        centerX = 500f,
        centerY = 800f,
        radius = 300f
    )
...
// Kai patrols back and forth (guard duty)
AnimatedCharacter(
    manifestation = kaiManifestation,
    assetBitmap = kaiAsset,
    behavior = MovementScenarios.kaiGuardPatrol()
)
...
AnimatedCharacter(
    manifestation = auraManifestation,
    assetBitmap = auraAsset,
    behavior = MovementPresets.customPath(
        waypoints = listOf(
            Pair(100f, 800f),
            Pair(500f, 600f),
            Pair(900f, 800f)
        ...
        loop = true,
        pauseAtEachPoint = 2.seconds
    )
)
```

**Movement Behaviors:**
- `casualWalkAcross()` - Walk from one side to the other
- `walkInAndWave()` - Enter, pause in center, leave
- `patrolBottom()` - March back and forth
- `randomWander()` - Walk to random points
- `nervousPacing()` - Short anxious back-and-forth
- `circlePatrol()` - Walk in a circle
- `walkInFromLeft/Right()` - Enter from off-screen
- `walkOutLeft/Right()` - Exit to off-screen

**Scenarios (Complete sequences):**
- `auraInvestigation()` - Aura walks in, looks around, investigates, leaves
- `kaiGuardPatrol()` - Kai marches back and forth like a sentry
- `meetInCenter()` - Two characters walk toward each other (duo scene!)
- `playfulChase()` - One character chases another

They don't just appear. **They walk into frame. They move around. They leave.**
